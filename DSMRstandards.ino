...
*/
  ...
  switch (line) {
    ...
    case 36:  // Equipment identifier BE (Gas)
              sprintf(telegram, "0-%d:96.1.1(37464C4F323131393039353734BE)\r\n", actGasMBus, val);
              break;
    case 37:  // gas_valve_position
              sprintf(telegram, "0-%d:24.4.0(%1d)\r\n", actGasMBus, (telegramCount%2));
              break;
    case 38:  // gas_delivered
              sprintf(telegram, "0-%d:24.2.3(%02d%02d%02d%02d%02d00S)(%s*m3)\r\n", actGasMBus 
                                                      ,(year() - 2000), month(), day(), hour(), minute() 
                                                      ,Format(GDelivered, 9, 3).c_str());
              break;
    case 39:  // mbus-Type [4]
    ...
    case 40:  // Equipment identifier BE (4)
              sprintf(telegram, "0-4:96.1.0(37464C4F3231314940494547BE40)\r\n", val);
              break;
    case 41:  // [4]_valve_position
              sprintf(telegram, "0-4:24.4.0(%1d)\r\n", (telegramCount%2));
              break;
    case 42:  // [4]
              sprintf(telegram, "0-4:24.2.1(%02d%02d%02d%02d%02d00S)(%s*m3)\r\n", (year() - 2000), month(), day(), hour(), minute(), 
                                                                            Format((GDelivered * 1.3), 9, 3).c_str());
              break;
    case 43:  sprintf(telegram, "!xxxx\r\n");   
              break;
  ...
  switch (line) {
    ...
    *******/
    ...
    case 13:  // Gas Device-Type
              sprintf(telegram, "0-%d:24.1.0(3)\r\n", actGasMBus, val);
              break;
    case 14:  // Equipment identifier (Gas)
              sprintf(telegram, "0-%d:96.1.0(4730303339303031363500000000000000)\r\n", actGasMBus, val);
              break;
    case 15:  // Last 5-minute value (temperature converted), gas delivered to client
              // in m3, including decimal values and capture time 
              sprintf(telegram, "0-%d:24.3.0(%02d%02d%02d%02d%02d00)(08)(60)(1)(0-%d:24.2.1)(m3)\r\n", actGasMBus
                                                  , (year() - 2000), month(), day(), hour(), minute()
                                                  , actGasMBus);
              break;
    case 16:  sprintf(telegram, "(%s)\r\n", Format(GDelivered, 9, 3).c_str());
              break;
    case 17:  // Gas valve position
              sprintf(telegram, "0-%d:24.4.0(1)\r\n", actGasMBus, val);
              break;
    case 18:  // meter Device-Type (4)
              sprintf(telegram, "0-4:24.1.0(005)\r\n", val);
              break;
    case 19:  // Equipment identifier (4)
              sprintf(telegram, "0-4:96.1.0(4730303339303031363500004444444444)\r\n", val);
              break;
    case 20:  // value (temperature converted), (4)
              sprintf(telegram, "0-4:24.3.0(%02d%02d%02d%02d%02d00)(08)(60)(1)(0-4:24.2.1)(m3)\r\n"
                                                  , (year() - 2000), month(), day(), hour(), minute());
              break;
    case 21:  sprintf(telegram, "(%s)\r\n", Format((GDelivered/3), 9, 3).c_str());
              break;
    case 22:  // valve position (4)
              sprintf(telegram, "0-4:24.4.0(1)\r\n", val);
              break;
    case 23:  sprintf(telegram, "!\r\n\r\n");     // just for documentation 
              break;
...
//--------------------------------------------------------------------
void readTelegramFromFile()
{
  bool eof = false, telegramEnd = false, telegramStart = false;
  
  if (!telegramFile) 
  {
    Debugf("open(%s, 'r') ", _TELEGRAM_FILE);
    telegramFile = FSYS.open(_TELEGRAM_FILE, "r"); // open for reading and writing
    if (!telegramFile) 
    {
  ...
  while (!telegramEnd) 
  {
    ...
    if (telegram[0] == '/') telegramStart = true;
    if (telegram[0] == '!') telegramEnd   = true;
    
    if (telegramStart)
    {
      Debugf("(%2d)[%s]\r\n", len, telegram);
      if (telegramEnd) Debugln("==EOT==");
      Serial.printf("%s\r\n", telegram);
    }
    yield();
